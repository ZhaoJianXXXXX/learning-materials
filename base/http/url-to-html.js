...
- 用户界面：主要包括工具栏、地址栏、前进/后退按钮、书签菜单、可视化页面加载进度、智能下载处理、首选项、打印等。
	除了浏览器主窗口显示请求的页面之外，其他显示的部分都属于用户界面。
- 浏览器引擎：是一个可嵌入的组件，其为渲染引擎提供高级接口;可以加载一个给定的URI，并支持诸如：前进/后退/重新加载等浏览操作;
	提供查看浏览会话的各个方面的挂钩，例如：当前页面加载进度、JavaScript alert;允许查询/修改渲染引擎设置。
- 渲染引擎：负责显示请求的内容。比如请求到HTML, 它会负责解析HTML、CSS并将结果显示到窗口中。
- 网络：用于网络请求, 如HTTP、FTP请求。它包括平台无关的接口和各平台独立的实现。
- UI后端：提供绘图和窗口原语，包括：用户界面控件集合、字体集合；
